  ...
  if (tier === 'in') {
    return (
      <div className={s.stockRow}>
        ...
        <span className={s.stockText}>{t('product.in_stock')} ({stock} {t('product.available')})</span>
      </div>
    );
  }
  if (tier === 'low') {
    return (
      <div className={s.stockRow}>
        ...
        <span className={`${s.stockText} ${s.stockLow}`}>{t('product.low_stock_prefix')} {stock} {t('product.low_stock_suffix')}</span>
      </div>
    );
  }
  return (
...
// is its own product), so this is inert until that data exists, per the
// requirement to support it "without a large category switch."
  ...
  return (
    <div className={s.buyBox}>
      <div className={s.badgesRow}>
        ...
        {hasDiscount && discountPercent != null && <Badge variant="error">−{discountPercent}%</Badge>}
      ...
      <div className={s.trustRow}>
        {warranty && (
          <span className={s.warrantyBadge}><ShieldCheck size={12} /> {warranty} {t('product.warranty_suffix')}</span>
        )}
      ...
      {product.tags?.length > 0 && (
        <div className={s.tagRow}>
          {product.tags.map((tag) => <span key={tag} className={s.tag}>{tag}</span>)}
        </div>
      )}
    </div>
  );
