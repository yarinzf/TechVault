import { useState, useCallback, useEffect, useRef } from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { LogIn, Smartphone } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';
import { useTranslation } from '../../context/LanguageContext';
import AuthBackground from './AuthBackground';
import AuthCloseButton from '../../components/ui/AuthCloseButton/AuthCloseButton';
import s from './LoginPage.module.css';

const COUNTRIES = [
  { code: '+972', label: '🇮🇱 +972' },
  { code: '+1',   label: '🇺🇸 +1'   },
  { code: '+44',  label: '🇬🇧 +44'  },
];

// ── Static icon components ─────────────────────────────────────────────────────
function TechVaultLogo() {
  return (
    <svg width="22" height="24" viewBox="0 0 133 142" fill="none" aria-hidden="true">
      <path d="M0 18.31L65.56 0L132.46 18.31L125.47 40.53L66.23 23.86L59.86 25.51L61.66 30.58L67.83 44.02L74 55.95L79.71 66.65L67.88 88.04L58.03 73L50.16 58.81L43.76 45.15L38.48 31.49L7.2 39.91L0 17.28" fill="#2563EB"/>
      <path d="M10.57 48.52L32.89 42.45L36.13 50.48L42.45 63.43L48.16 73.92L58.03 89.97L68.06 104.16L77.62 89.5L83.49 79.17L90.43 65.59L96.91 52.02L100.76 42.45L122.67 48.78L116.5 64.05L106.47 84.88L94.44 105.86L80.4 126.37L68.83 141.34L51.71 121.44L41.1 106.32L31.96 91.9L24.86 79.01L15.45 59.11L10.57 48.52Z" fill="#2563EB"/>
    </svg>
  );
}

function GoogleIcon() {
  return (
    <svg width="17" height="17" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
    </svg>
  );
}

function AppleIcon() {
  return (
    <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
    </svg>
  );
}

// ── Component ──────────────────────────────────────────────────────────────────
export default function LoginPage() {
  const { login, loginWithGoogle, loginWithApple, smsStart, smsVerify } = useAuth();
  const navigate  = useNavigate();
  const location  = useLocation();
  const from      = location.state?.from?.pathname || '/';
  const t         = useTranslation();

  // login mode: 'email' | 'sms'
  const [loginMode, setLoginMode] = useState('email');
  const [fading,    setFading]    = useState(false);
  const [form,      setForm]      = useState({ email: '', password: '', phone: '', country: '+972' });
  const [smsOtp,    setSmsOtp]    = useState('');
  const [smsSent,   setSmsSent]   = useState(false); // true after smsStart succeeds
  const [error,     setError]     = useState('');
  const [loading,   setLoading]   = useState(false);

  const handle = (field) => (e) => setForm(f => ({ ...f, [field]: e.target.value }));

  // ── Google Sign In via renderButton overlay ───────────────────────────────
  // prompt() is suppressed by the browser after repeated dismissals; renderButton
  // always opens the account-chooser popup on a real user click.
  const gsiContainerRef = useRef(null);
  const gsiCallbackRef  = useRef(null);

  // Keep callback ref up-to-date without re-running the effect
  gsiCallbackRef.current = async (response) => {
    setLoading(true);
    setError('');
    try {
      await loginWithGoogle(response.credential);
      navigate(from, { replace: true });
    } catch (err) {
      setError(err.message || t('auth.google_failed'));
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    if (!import.meta.env.VITE_GOOGLE_CLIENT_ID) return;
    const tryRender = () => {
      if (!window.google?.accounts?.id || !gsiContainerRef.current) return false;
      window.google.accounts.id.initialize({
        client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID,
        callback: (r) => gsiCallbackRef.current?.(r),
        auto_select: false,
        cancel_on_tap_outside: false,
      });
      window.google.accounts.id.renderButton(gsiContainerRef.current, {
        type: 'standard',
        size: 'large',
        width: gsiContainerRef.current.offsetWidth || 120,
      });
      return true;
    };
    if (tryRender()) return;
    // Script still loading — attach load listener
    const script = document.querySelector('script[src*="accounts.google.com/gsi"]');
    if (!script) return;
    script.addEventListener('load', tryRender);
    return () => {
      script.removeEventListener('load', tryRender);
      window.google?.accounts?.id?.cancel?.();
    };
  }, []);

  // ── Toggle SMS mode ────────────────────────────────────────────────────────
  const toggleSMS = useCallback(() => {
    setFading(true);
    setTimeout(() => {
      setLoginMode(m => {
        const next = m === 'email' ? 'sms' : 'email';
        if (next === 'email') {
          setSmsSent(false);
          setSmsOtp('');
        }
        return next;
      });
      setError('');
      setFading(false);
    }, 200);
  }, []);

  // ── Main form submit ───────────────────────────────────────────────────────
  const submit = async (e) => {
    e.preventDefault();
    setError('');
    setLoading(true);
    try {
      if (loginMode === 'email') {
        await login(form.email, form.password);
        navigate(from, { replace: true });

      } else if (loginMode === 'sms' && !smsSent) {
        // Step 1 — send OTP
        const fullPhone = `${form.country}${form.phone.replace(/^0/, '')}`;
        await smsStart(fullPhone);
        setSmsSent(true);

      } else if (loginMode === 'sms' && smsSent) {
        // Step 2 — verify OTP
        const fullPhone = `${form.country}${form.phone.replace(/^0/, '')}`;
        await smsVerify(fullPhone, smsOtp);
        navigate(from, { replace: true });
      }
    } catch (err) {
      setError(err.message || t('auth.login_failed'));
    } finally {
      setLoading(false);
    }
  };

  // ── Apple Sign In ──────────────────────────────────────────────────────────
  const handleApple = async () => {
    if (!window.AppleID?.auth) {
      setError(t('auth.apple_script_error'));
      return;
    }
    setLoading(true);
    setError('');
    try {
      window.AppleID.auth.init({
        clientId:    import.meta.env.VITE_APPLE_CLIENT_ID,
        scope:       'name email',
        redirectURI: import.meta.env.VITE_APPLE_REDIRECT_URI || window.location.origin + '/login',
        usePopup:    true,
      });
      const response = await window.AppleID.auth.signIn();
      await loginWithApple(response.authorization.id_token);
      navigate(from, { replace: true });
    } catch (err) {
      if (err?.error !== 'popup_closed_by_user') {
        setError(err.message || t('auth.apple_failed'));
      }
    } finally {
      setLoading(false);
    }
  };

  const isSMS = loginMode === 'sms';

  // Submit button label
  let submitLabel = t('auth.login_btn');
  if (loading)              submitLabel = t('auth.logging_in');
  else if (isSMS && !smsSent) submitLabel = t('auth.sms_send_code_btn');
  else if (isSMS && smsSent)  submitLabel = t('auth.sms_verify_btn');

  return (
    <div className={s.page}>
      {/* Real storefront rendered behind the blur */}
      <div className={s.bgLayer} aria-hidden="true">
        <AuthBackground />
      </div>
      <div className={s.overlay} aria-hidden="true" />

      <div className={s.cardWrap}>
      <div className={s.card}>

        <AuthCloseButton />

        {/* Logo */}
        <Link to="/" className={s.logo} aria-label={t('auth.logo_home_label')}>
          <TechVaultLogo />
          <span>Tech<span className={s.logoAccent}>Vault</span></span>
        </Link>

        {/* Pill tabs */}
        <div className={s.tabs} role="tablist">
          <span className={`${s.tab} ${s.tabActive}`} role="tab" aria-selected="true">
            {t('auth.login_btn')}
          </span>
          <Link to="/register" className={s.tab} role="tab" aria-selected="false">
            {t('auth.create_btn')}
          </Link>
        </div>

        {/* Form */}
        <form className={s.form} onSubmit={submit} noValidate>
          {error && <div className={s.error} role="alert">{error}</div>}

          {/* Animated field panel */}
          <div className={`${s.panel} ${fading ? s.panelHidden : ''}`}>

            {/* ── Email mode ── */}
            {!isSMS && (
              <>
                <div className={s.field}>
                  <label className={s.label} htmlFor="login-email">{t('auth.email')}</label>
                  <input
                    id="login-email"
                    className={s.input}
                    type="email"
                    placeholder="jane@example.com"
                    value={form.email}
                    onChange={handle('email')}
                    required
                    autoComplete="email"
                    dir="ltr"
                  />
                </div>
                <div className={s.field}>
                  <label className={s.label} htmlFor="login-password">{t('auth.password')}</label>
                  <input
                    id="login-password"
                    className={s.input}
                    type="password"
                    placeholder="••••••••"
                    value={form.password}
                    onChange={handle('password')}
                    required
                    autoComplete="current-password"
                    dir="ltr"
                  />
                  <button type="button" className={s.forgot} onClick={() => navigate('/forgot-password')}>{t('auth.forgot_password_link')}</button>
                </div>
              </>
            )}

            {/* ── SMS mode — step 1: phone number ── */}
            {isSMS && !smsSent && (
              <div className={s.field}>
                <label className={s.label} htmlFor="login-phone">{t('auth.phone_label')}</label>
                <div className={s.phoneRow}>
                  <select
                    className={`${s.input} ${s.countrySelect}`}
                    value={form.country}
                    onChange={handle('country')}
                    dir="ltr"
                    aria-label={t('auth.country_code_label')}
                  >
                    {COUNTRIES.map(c => (
                      <option key={c.code} value={c.code}>{c.label}</option>
                    ))}
                  </select>
                  <input
                    id="login-phone"
                    className={`${s.input} ${s.phoneInput}`}
                    type="tel"
                    placeholder="05X-XXXXXXX"
                    value={form.phone}
                    onChange={handle('phone')}
                    autoComplete="tel"
                    dir="ltr"
                    required
                  />
                </div>
              </div>
            )}

            {/* ── SMS mode — step 2: OTP code ── */}
            {isSMS && smsSent && (
              <div className={s.field}>
                <label className={s.label} htmlFor="login-otp">{t('auth.otp_label')}</label>
                <p className={s.smsHint}>
                  {t('auth.otp_sent_to')}{form.country}{form.phone}
                  {' · '}
                  <button
                    type="button"
                    className={s.smsResend}
                    onClick={async () => {
                      setError('');
                      setLoading(true);
                      try {
                        const fullPhone = `${form.country}${form.phone.replace(/^0/, '')}`;
                        await smsStart(fullPhone);
                      } catch (err) {
                        setError(err.message || t('auth.otp_resend_failed'));
                      } finally {
                        setLoading(false);
                      }
                    }}
                  >
                    {t('auth.otp_resend_btn')}
                  </button>
                </p>
                <input
                  id="login-otp"
                  className={s.input}
                  type="text"
                  inputMode="numeric"
                  pattern="[0-9]*"
                  placeholder="123456"
                  value={smsOtp}
                  onChange={e => setSmsOtp(e.target.value)}
                  autoComplete="one-time-code"
                  dir="ltr"
                  maxLength={6}
                  required
                  autoFocus
                />
              </div>
            )}

          </div>{/* /panel */}

          <button
            type="submit"
            className={s.submit}
            disabled={loading}
            aria-busy={loading}
          >
            <LogIn size={16} />
            {submitLabel}
          </button>
        </form>

        {/* divider */}
        <div className={s.divider}>{t('auth.or_continue_with')}</div>

        {/* OAuth row: Google · Apple · SMS */}
        <div className={s.oauth}>
          <div className={s.oauthGoogleWrap}>
            <button
              type="button"
              className={s.oauthBtn}
              disabled={loading}
              aria-label={t('auth.google_signin_label')}
              tabIndex={-1}
            >
              <GoogleIcon /> Google
            </button>
            <div
              ref={gsiContainerRef}
              className={s.gsiOverlay}
              style={loading ? { pointerEvents: 'none' } : undefined}
              aria-hidden="true"
            />
          </div>
          <button
            type="button"
            className={`${s.oauthBtn} ${!import.meta.env.VITE_APPLE_CLIENT_ID ? s.oauthBtnDisabled : ''}`}
            onClick={handleApple}
            disabled={loading || !import.meta.env.VITE_APPLE_CLIENT_ID}
            aria-label={t('auth.apple_signin_label')}
            title={!import.meta.env.VITE_APPLE_CLIENT_ID ? t('auth.apple_not_configured') : undefined}
          >
            <AppleIcon /> Apple
          </button>
          <button
            type="button"
            className={`${s.oauthBtn} ${isSMS ? s.oauthBtnActive : ''}`}
            onClick={toggleSMS}
            disabled={loading}
            aria-label={t('auth.sms_signin_label')}
            aria-pressed={isSMS}
          >
            <Smartphone size={16} /> SMS
          </button>
        </div>

        {/* Terms */}
        <div className={s.terms}>
          {t('auth.terms_prefix')}<Link to="/terms">{t('auth.terms_link')}</Link>{t('auth.terms_middle')}<Link to="/privacy">{t('auth.privacy_link')}</Link>{t('auth.terms_suffix')}
        </div>

        {/* Footer */}
        <div className={s.footer}>
          {t('auth.no_account')}{' '}
          <Link to="/register" className={s.link}>{t('auth.create_account')}</Link>
        </div>

        {/* Dev-only credentials hint */}
        <div className={s.devHint}>
          {t('auth.dev_demo')}: <strong>alice@example.com</strong> / <strong>User123!</strong><br />
          {t('auth.dev_admin')}: <strong>admin@techvault.dev</strong> / <strong>Admin123!</strong><br />
          {t('auth.dev_warehouse')}: <strong>warehouse@techvault.dev</strong> / <strong>Admin123!</strong><br />
          {t('auth.dev_super_admin')}: <strong>superadmin@techvault.dev</strong> / <strong>Admin123!</strong>
        </div>

      </div>
      </div>{/* cardWrap */}
    </div>
  );
}
