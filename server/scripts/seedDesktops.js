...
/**
 * Desktop seed — inserts / updates branded desktop PC products.
 *
 * Usage:
 *   npm run seed:desktops            # safe: skips existing SKUs
 *   npm run seed:desktops:force      # overwrites existing desktop products
 *
 * Requires `npm run seed` to have been run first (creates categories).
 * Brand data files: server/data/desktops/<brand>.desktops.js
 */
...
const { normalize } = require('../utils/catalog/desktopSpecNormalizer');
...
const loadBrand = (name) => {
  try { return require(`../data/desktops/${name}.desktops`); }
  catch { return []; }
};
...
async function main() {
  ...
  const cat = await Category.findOne({ slug: 'desktops' }).select('_id').lean();
  if (!cat) {
    fail('Desktops category not found. Run `npm run seed` first to create categories.');
    process.exit(1);
  }

  const rawAll = BRAND_FILES.flatMap(loadBrand);
  if (rawAll.length === 0) {
    warn('No desktop data files found in server/data/desktops/. Add brand files and re-run.');
    process.exit(0);
  }
  ...
  console.log('');
  console.log(c.bold('─── Desktop Seed Summary ──────────────────────────────'));
