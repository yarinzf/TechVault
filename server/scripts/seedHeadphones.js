...
/**
 * Headphone seed — inserts / updates headphone products.
 *
 * Usage:
 *   npm run seed:headphones            # safe: skips existing SKUs
 *   npm run seed:headphones:force      # overwrites existing headphone products
 *
 * Requires `npm run seed` to have been run first (creates categories).
 * Brand data files: server/data/headphones/<brand>.headphones.js
 */
...
const { normalize } = require('../utils/catalog/headphoneSpecNormalizer');
...
const loadBrand = (name) => {
  try { return require(`../data/headphones/${name}.headphones`); }
  catch { return []; }
};
...
async function main() {
  ...
  if (rawAll.length === 0) {
    warn('No headphone data files found in server/data/headphones/. Add brand files and re-run.');
    process.exit(0);
  }
  ...
  console.log('');
  console.log(c.bold('─── Headphone Seed Summary ────────────────────────────'));
