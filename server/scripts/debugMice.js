...
/**
 * Debug script — mouse product inventory report.
 * Usage: npm run debug:mice
 */
...
async function main() {
  ...
  console.log('');
  console.log(c.bold('══════════════════════════════════════════════════════'));
  console.log(c.bold('  TechVault — Mouse Debug Report'));
  console.log(c.bold('══════════════════════════════════════════════════════'));
  console.log('');

  // ── 1. Data files ─────────────────────────────────────────────────────────────
  console.log(c.bold('── Data files (server/data/mice/) ──'));
  let rawTotal = 0, filesFound = 0, filesSkipped = 0;

  for (const brand of BRAND_FILES) {
    try {
      const products = require(`../data/mice/${brand}.mice`);
  ...
  // ── 2. Category ───────────────────────────────────────────────────────────────
  console.log('');
  console.log(c.bold('── MongoDB — Mice category ──'));
  const cat = await Category.findOne({ slug: 'mice' }).lean();
  if (!cat) {
    console.log(`  ${c.red('✗')} Category "mice" NOT FOUND. Run ${c.yellow('npm run seed')} first.`);
  } else {
  ...
  if (cat) {
    ...
    if (first10.length === 0) {
      console.log(`  ${c.yellow('No mouse products found. Run npm run seed:mice')}`);
    } else {
      first10.forEach((p, i) => {
        const flags = [
    ...
    // ── Missing TV-MS-XXXX SKU ranges ────────────────────────────────────────
    console.log('');
    console.log(c.bold('── Missing TV-MS-XXXX SKU ranges ──'));
    const allSkus = await Product.find({ category: catId }).select('sku').lean();
    const tvMsNums = new Set();
    for (const { sku } of allSkus) {
      const m = sku.match(/^TV-MS-(\d+)$/);
      if (m) tvMsNums.add(parseInt(m[1], 10));
    }
    const maxSku  = tvMsNums.size > 0 ? Math.max(...tvMsNums) : 0;
    const missing = [];
    for (let n = 1; n <= Math.max(maxSku, 460); n++) {
      if (!tvMsNums.has(n)) missing.push(n);
    }
    if (missing.length === 0) {
      console.log(`  ${c.green('✓')} No gaps in TV-MS-0001 → TV-MS-${String(maxSku).padStart(4, '0')}`);
    } else {
      const ranges = [];
      ...
      ranges.forEach(([s, e]) => {
        const label = s === e
          ? `TV-MS-${String(s).padStart(4, '0')}`
          : `TV-MS-${String(s).padStart(4, '0')} → TV-MS-${String(e).padStart(4, '0')}`;
    ...
    if (total === 0) {
      console.log(`  ${c.red('✗')} No mouse products in MongoDB.`);
      console.log(`    ${c.yellow('Fix:')} Run ${c.bold('npm run seed:mice')}`);
    } else if (total < rawTotal) {
      console.log(`  ${c.yellow('⚠')} Only ${total} of ${rawTotal} products are in MongoDB.`);
      console.log(`    ${c.yellow('Fix:')} Run ${c.bold('npm run seed:mice')} to add missing products.`);
    } else {
    ...
    console.log(`  ${c.red('✗')} Cannot query products — mice category is missing.`);
  }
